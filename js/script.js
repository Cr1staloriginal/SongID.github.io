...
// ========== ПОИСК ТРЕКА ЧЕРЕЗ ITUNES (ОСНОВНОЙ ДЛЯ РФ) ==========
async function searchTrackITunes(query) {
  const url = `https://itunes.apple.com/search?term=${encodeURIComponent(query)}&limit=5&entity=song`;
...
// ========== ПОИСК ТРЕКА ЧЕРЕЗ DEEZER (РЕЗЕРВНЫЙ) ==========
async function searchTrackDeezer(query) {
...
// ========== СПОТИФАЙ (ДЛЯ BPM, ЭНЕРГИИ, ЯРКОСТИ) ==========
...
function classifyGenre(features, tags = []) {
  ...
  if (tags && tags.length > 0) {
    ...
    return {
      ...
      confidence: 70,
      alternatives: [],
      explanation: `Жанр определён по тегам: ${translatedTags.join(', ')}.`
    };
  }
...
searchInput.addEventListener('input', async () => {
  ...
  if (!q) return;
  searchResults.innerHTML = '<p class="meta">Ищем в iTunes...</p>';

  try {
    let tracks = await searchTrackITunes(q);
    
    if (!tracks.length) {
      searchResults.innerHTML = '<p class="meta">Ищем в Deezer...</p>';
      tracks = await searchTrackDeezer(q);
    }
    ...
    tracks.forEach(track => {
      ...
      el.innerHTML = `<strong>${track.artist} — ${track.title}</strong><br><span class="meta">Источник: ${track.bpm ? 'iTunes' : 'Deezer'}</span>`;
      
      el.addEventListener('click', async () => {
        el.innerHTML = `<strong>${track.artist} — ${track.title}</strong><br><span class="meta">Анализируем...</span>`;
        el.disabled = true;

        let stats = await getSpotifyTrackInfo(track.artist, track.title);
        if (!stats) {
          stats = { bpm: track.bpm || 120, energy: 0.5, brightness: 0.5 };
        }

        const result = classifyGenre(stats);
        showResult(result, `Источник: ${stats.bpm === 120 && stats.energy === 0.5 ? 'iTunes (BPM)' : 'Spotify'} — ${track.artist} / ${track.title}.`);
    ...
    searchResults.innerHTML = '<p class="meta warn">Ошибка при поиске. Проверьте консоль.</p>';
